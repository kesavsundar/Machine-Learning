...
class nGaussian:
    def __init__(self):
        self.data = np.loadtxt(open("/home/kesavsundar/machinelearning/ml/EM/2gaussian.txt", 'rb'),
        ...
        self.gamma = np.ones_like(self.data)
        self.ll = 0.0
        self.k = 2
        self.n_features = 2
    ...
    def random_assignment(self):
        ...
        self.sd = [np.matrix([[0.5, 1], [0.25, 3]]), np.matrix([[0.7, 0.2], [1, 1]])]
        self.w = [float(0.4), float(0.6)]
        return
    ...
    def pdf(self, comp):
        ...
        for row in range(0, len(self.data)):
            x_minus_mean = np.matrix(self.data[row, :] - self.mean[comp, :]).reshape(1, self.k)
    ...
    def calc_log_likelihood(self):
        ...
        sum_likelihood = self.score[:, 0] + self.score[:, 1]
    ...
    def update_parameters(self):
        ...
        initMeans = np.zeros(4).reshape(2, 2)
        #[np.matrix([[0], [0]]), np.matrix([[0], [0]])]
        initCovariances = [np.matrix([[0, 0],[0, 0]]), np.matrix([[0, 0],[0, 0]])]
        ...
        new_sd = [(initCovariances[0]/n0), (initCovariances[1]/n1)]
        self.sd = new_sd
        return
